    ...
    public function statistiche()
    {
        ...
        $stats = [
            ...
            'pagine_visitate' => collect($logs)
                ...
                ->filter(fn($log) => $log['url'] !== 'dashboard/logs')
                ->filter(fn($log) => $log['url'] !== 'dashboard/logs?search=')
                ->filter(fn($log) => !str_starts_with($log['url'], 'dashboard/logs?'))
                ->filter(fn($log) => $log['url'] !== 'https://testandriani.promomedia.online/dashboard/logs')
                ->filter(fn($log) => !str_starts_with($log['url'], 'https://testandriani.promomedia.online/dashboard/logs?'))
                ->filter(fn($log) => $log['url'] !== 'dashboard/errori')
                ->filter(fn($log) => $log['url'] !== 'dashboard/errori?search=')
                ->filter(fn($log) => !str_starts_with($log['url'], 'dashboard/errori?'))
                ->filter(fn($log) => $log['url'] !== 'https://testandriani.promomedia.online/dashboard/errori')
                ->filter(fn($log) => !str_starts_with($log['url'], 'https://testandriani.promomedia.online/dashboard/errori?'))
                ->groupBy('url')
                ->map->count()
                ->sortDesc()
        ];
